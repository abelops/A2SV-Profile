        ...
        <div className="flex ">
            ...
            <div className="flex flex-col ml-10 text-left mt-4">
                ...
                <div className='flex gap-2 pb-3 border-orange-400 border-b-2'>
                    {languages.map((lang:string)=>{return(<span key={Math.random()} className='text-gray-600'>{lang}</span>)})}
                </div>
            </div>
        ...
        <div className="w-full h-100 shadow-md rounded-lg bg-slate-50 text-black relative flex">
            ...
            <div className="px-4 py-4 my-10 mx-10 flex flex-col">
                ...
                (<div className='flex gap-2 mt-10'>
                    {tags.map((tag)=>{
                        return(<span key={Math.random()} className='px-4 py-2 bg-orange-400 text-white rounded-full text-sm'>{tag}</span>)
                    })}
                </div>): "No tags"}
            </div>
            <Contact phone={phone}/>
